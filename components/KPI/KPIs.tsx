...
export const KPI = ({ stats }: NumberKPIProps) => {
  return (
    <div>
      <dl className="grid grid-cols-1 divide-y divide-gray-100 overflow-hidden rounded-xl shadow-sm md:grid-cols-3 md:divide-x md:divide-gray-100 md:divide-y-0  bg-white border border-gray-100">
        {stats.map((item) => (
          <Link
            href={item?.href ?? ""}
            key={item.name}
