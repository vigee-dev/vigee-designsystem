...
function useStepper<StepContent extends string>(
  ...
  const goToStep = (step: number) =>
    setCurrentStep(Math.max(0, Math.min(step, steps.length - 1))); // Ajout de la méthode goToStep
    ...
    <Container className={cn("p-6", className)}>
      <div className="flex justify-between">
        {stepper.steps?.map((step, i) => (
          <div
            key={i}
            className={`step-item ${stepper.currentStep === i ? "active" : ""} ${i < stepper.currentStep ? "complete" : ""}`}
          >
            <div className="step">
              {i < stepper.currentStep ? <PiCheckTickCircleContrast /> : i + 1}
            </div>
            <p
              className={`${stepper.currentStep === i ? "text-black" : "text-gray-500"} text-xs md:text-sm font-medium pt-1`}
            >
              {step}
            </p>
          </div>
