...
const Content = ({ title, subtitle, icon, children, className }: Omit<PageHeaderProps, "href" | "onClick">) => (
  <div
    className={cn(
      "rounded-xl h-fit py-2 md:my-2 text-primary hover:opacity-80 items-center border-none  group transition-ease-in-out duration-100 hover:cursor-pointer bg-transparent transition-ease-in-out ",
      className
    )}>
    <div className="flex justify-between  w-full items-center ">
      <div className="flex gap-x-8 items-center">
        ...
        <div className="flex flex-col px-2">
          ...
          {subtitle && <p className=" md:text-sm text-xs transition-ease-in-out duration-100 max-w-48 md:max-w-none">{subtitle}</p>}
        </div>
      </div>
      <div className="flex gap-x-4  p-1 ">{children ? children : <PiChevronRightStroke className="text-primary group-hover:text-primary-light transform ease-in-out duration-300 h-6 w-6" />}</div>
    </div>
  </div>
);
