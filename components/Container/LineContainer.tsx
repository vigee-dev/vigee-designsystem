...
const Content = ({
  ...
  <div
    className={cn(
      "rounded-xl h-fit py-2 md:my-2 text-gray-800 hover:opacity-60 items-center border-none  group transition-ease-in-out duration-100 hover:cursor-pointer bg-transparent transition-ease-in-out ",
      className
    )}
  >
    <div className="flex justify-between  w-full items-center ">
      <div className="flex gap-x-8 items-center">
        ...
        <div className="flex flex-col px-2">
          <TypographyH4 className="text-xs md:text-base  font-bold   ">
            {title}
          </TypographyH4>
          {subtitle && (
      ...
      <div className="flex gap-x-4  p-1 ">
        {children ? (
          ...
          <PiChevronRightStroke className="text-primary group-hover:text-primary-light transform ease-in-out duration-300 h-6 w-6" />
        )}
      </div>
    </div>
  </div>
